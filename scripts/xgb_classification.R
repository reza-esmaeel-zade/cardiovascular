...
## patients_data <- patients_data%>%select(-targetName,targetName); edited by reza not needed actually;
...
for(exe_i in 1:execution_number)
{
  ...
  # Allocation of the size of the training set; edited 80 to 70 by reza
  ...
  training_labels <- patients_data[1:size_training_set, target_index]   
  test_labels <- patients_data[test_set_index_start:test_set_index_end, target_index]
  ...
  this_max_depth <- 3 #edited from 2 to 3 by reza
  this_eta <- 1
  this_nthread <- 2
  this_nrounds <- 2
  this_verbose <- 0
...
target_csv <- paste("./results/","xgb_classification_",execution_number,"_exec.csv", sep = "") 
