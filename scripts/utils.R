...
barPlotOfRanking <- function(rankingDataFrame, valuesToRank, featuresCol, positionCol, exe_num, x_label, y_label, x_upper_lim) 
{
  ...
  thisPngFile <-  paste("./results/", y_label, "_features_", exe_num, ".png", sep="")
  png(thisPngFile, width = 1920, height = 1080, pointsize = 20)
  p <- ggplot(data=rankingDataFrame, aes(x=reorder(featuresCol, -positionCol), y=valuesToRank)) +
    geom_bar(stat="identity", fill="steelblue") +
    labs(title = paste("Feature ranking on ", y_label, sep=""), y = y_label, x = x_label) +
...
train_data_balancer <- function(thisDataset, target_index, training_set_perc, INPUT_PERC_POS, balancedFlag) {
  ...
  training_set_numb_of_element <- round(training_set_perc*thisDatasetSize/100,0)
  cat("\nThe training set will contain ", training_set_numb_of_element, " items (", training_set_perc, "%) of the data instances\n", sep="")
  
  test_set_perc <- 100-training_set_perc
  test_set_numb_of_ele <- thisDatasetSize - training_set_numb_of_element
  ...
  if (balancedFlag == TRUE) {
    ...
    training_set_numb_of_element <- round((positiveSetSize+negativeSetSize)*training_set_perc/100,0)
    test_set_numb_of_ele <- (positiveSetSize+negativeSetSize) - training_set_numb_of_element
  ...
  # cat("\nThe training set will contain ", training_set_numb_of_element, " items", sep="")
  # cat("\nThe test set will contain ", test_set_numb_of_ele, " items \n", sep="")
  
  # newTrainingSet <- 50% positive_subset & 50% negative_subset 
  # from index 1 to 81 (that is training_set_numb_of_element/2 ) of positive_subset
  # and from index 1 to 81 (that is training_set_numb_of_element/2 ) of negative_subset
  
  train_set_num_of_positives <- round(training_set_numb_of_element*(INPUT_PERC_POS/100), 0)
  # cat("INPUT_PERC_POS = ", INPUT_PERC_POS, "%\n", sep="")
  # cat("train_set_num_of_positives = ", train_set_num_of_positives, "\n", sep="")
  train_set_num_of_negatives <- round(training_set_numb_of_element - train_set_num_of_positives,0)
  ...
  # from index 82 (that is training_set_numb_of_element/2 + 1) to the end of positive_subset
  # and from index 82 (that is training_set_numb_of_element/2 + 1) to the end of negative_subset
