...
for(exe_i in 1:execution_number)
{
  ...
  maxK <- 20 #edited by reza; small!!!
  
  mcc_array <- character(length(maxK))
  
  
  c_array = c(0.001, 0.01, 0.1, 1, 10, 20) #edited by reza
  mccCounter = 1
  ...
  for(thisC in c_array)
  {
    ...
    svm_model <- svm(patients_data_train_labels ~ ., cost=thisC, data=patients_data_train, type = "nu-classification", kernel = "linear") #edited to nu-classification by reza
  ...
  svm_model_new <- svm(patients_data_train_labels ~ ., cost=c_array[bestCindex], data=patients_data_train, type = "nu-classification", kernel = "linear") #edited to nu class. by reza
...
target_csv <- paste("./results/","svm_lin_classification_",execution_number,"_exec.csv", sep = "") 
