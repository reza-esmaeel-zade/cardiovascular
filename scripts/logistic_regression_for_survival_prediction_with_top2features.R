...
patients_data <- patients_data %>%
  dplyr:: select(-targetName,targetName)
...
for(exe_i in 1:execution_number)
{
  ...
  nonTimeFeatures <- grep("^[^factor*]", row.names(clinical_feature_ranking), value=TRUE)
...
target_csv <- paste("./results/","log_reg_top_2_features_",execution_number,"_exec.csv", sep = "") 

write.csv(dec_three(meanRowResults), target_csv)
