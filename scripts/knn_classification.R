...
for(exe_i in 1:execution_number)
{
  ...
  training_set_percentagee <- 60
  cat("training_set_percentagee = ", training_set_percentagee, "% \n", sep="")
  validation_set_percentage <- 20
  cat("validation_set_percentage = ", validation_set_percentage, "% \n", sep="")
  test_set_percentage <- 100 - training_set_percentagee - validation_set_percentage
  cat("test_set_percentage = ", test_set_percentage, "% \n", sep="")
  
  # the training set is the first 60% of the whole dataset
  training_set_first_index <- 1 # NEW
  training_set_last_index <- round(dim(patients_data)[1]*training_set_percentagee/100) # NEW
  
  # the validation set is the following 20% of the whole dataset
  validation_set_first_index <- round(dim(patients_data)[1]*training_set_percentagee/100)+1 # NEW
  validation_set_last_index <- round(dim(patients_data)[1]*(training_set_percentagee+validation_set_percentage)/100) # NEW
  
  # the test set is the last 20% of the whole dataset
  test_set_first_index <- round(dim(patients_data)[1]*(training_set_percentagee+validation_set_percentage)/100)+1 # NEW
  ...
  maxK <- 40 #NEW edited by reza ;too biiiiig!
...
target_csv <- paste("./results/","knn_",execution_number,"_exec.csv", sep = "") 
