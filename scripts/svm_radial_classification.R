...
execution_number <- 100 #edited by reza
...
for(exe_i in 1:execution_number)
{
  ...
  c_array = c(0.001, 0.01, 0.1, 1, 10, 20)
  mccCounter = 1
  ...
  for(thisC in c_array)
  {
    ...
    svm_model <- svm(patients_data_train_labels ~ ., cost=thisC, data=patients_data_train, type = "C-classification", kernel = "radial")
  ...
  svm_model_new <- svm(patients_data_train_labels ~ ., cost=c_array[bestCindex], data=patients_data_train, type = "C-classification", kernel = "linear")
...
target_csv <- paste("./results/","svm_rad_classification_",execution_number,"_exec.csv", sep = "") 
